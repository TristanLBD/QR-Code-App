<?php
    $QrCodeType = "text";
    $Emoji = "fa-solid fa-file-lines";
    $actionType = "Créer";
    $QrCodeTitle = 'Texte';
    if(isset($_GET['type'])) {
        $availableQrCodeType = ["lien","text","mail","telephone"];
        if(in_array(htmlspecialchars($_GET['type']),$availableQrCodeType)) { $QrCodeTitle = htmlspecialchars($_GET['type']); $QrCodeType = htmlspecialchars($_GET['type']); }
        if(htmlspecialchars($_GET['type']) == "lien") { $Emoji = "fa-solid fa-link"; }
        if(htmlspecialchars($_GET['type']) == "mail") { $Emoji = "fa-solid fa-envelope"; }
        if(htmlspecialchars($_GET['type']) == "telephone") { $Emoji = "fa-solid fa-phone"; $QrCodeTitle = "téléphone";}
    }
    ...
    if(isset($_GET["id"]) && is_numeric($_GET['id']) && $_GET["id"] > 0  && file_exists('./assets/QRImages/'.$_GET['id'].'.png')) {
        ...
        if($values) {
            $firstInputValue = !empty($values['lien']) ? $values['lien'] : (!empty($values['mail']) ? $values['mail'] : (!empty($values['text']) ? $values['text'] : $values['telephone']));
        }
    }
...
?>
    ...
    <div class="row">
        <div class="col-12 col-md-8">
            <div id="title-border"><span><?= $actionType ?> un <?= $QrCodeTitle ?></span></div>
            <form method="post" action="generateqrcode.php" id="qrForm" enctype="multipart/form-data">
                
                <div class="row">
                    <div class="form-control-group col-12 mt-3">
                        <label for="input-value" class="form-label fw-bolder"><i class="<?= $Emoji; ?>"></i> <?= ucfirst($QrCodeTitle); ?> :</label>
                        <input value="<?= $firstInputValue ?>" type="mail" class="form-control" name="input-value" id="input-value">
                    </div>
                </div>
        ...
        <div class="col-12 col-md-4 d-flex flex-column justify-content-center align-items-center">
            <div class="row">
                <div class="col">
                    <p class="fs-1 fw-bolder text-decoration-underline"><?= ucfirst($QrCodeTitle); ?></p>
                </div>
            </div>
            <div class="row">
                <div class="col">
        ...
        document.addEventListener("DOMContentLoaded", function () {
            $(document).ready(function() {
                $("#qrForm").submit(function(event) {
                    ...
                    formData.append('input-value', $('#input-value').val());
                    ...
                    var requiredFields = ['input-value'];
